...
namespace Interface_2
{
    public class AdjacencySetGraph
    {
        ...
        public int selectMaxVertex(List<int> value, List<bool> processed, int size) //gets the vertex that has the lowest value
        {
            int maximum = -1;
            int vertex = 0;
            for (int i = 0; i < size; ++i)
            {
                if (processed[i] == false && value[i] > maximum)
                {
                    vertex = i;
                    maximum = value[i];
                }
        ...
        public Tuple<List<int>, int> DijkstrasAlgorithmLong(int startVertex, int endVertex) //returns tuple 1) path, 2) cost
        {
            if (!IsInVertexList(startVertex))
            {
                throw new Exception("Start vertex does not exist.");
            ...
            for (int i = 0; i < size; ++i)
            {
                value.Add(-2);//populate with maximum number
            }
            parent[startVertex] = -1; //begin with source node
            value[startVertex] = 0;
            for (int row = 0; row < size - 1; ++row)
            {
                //choose the next minimum vertex and mark it as processed
                int minVertex = selectMaxVertex(value, processed, size);
                processed[minVertex] = true;
                for (int column = 0; column < size; ++column)
                {
                    //checks for a present edge by comparing it with -1
            ...
            int currentVertex = endVertex;
            path.Add(endVertex);//add the vertex to the path list
            while (currentVertex != startVertex)
            {
                try
                {
                ...
                catch (ArgumentOutOfRangeException) //where this happens, return null to indicate no path
                {
                    return null;
                }
            }
            //path is generated in reverse order so reverse the order
            path.Reverse();
            int cost = 0;
            for (int i = 0; i < path.Count() - 1; ++i)
            {
